link bw16 firmware for flipper mode: https://github.com/gorebrau/delfyRTL


support vietnamese and filipino


add tetris game in other menu


baoduscuvaomomnhi







## ⚠️ Disclaimer and Legal Warning

This firmware, software, and all associated tools (including but not limited to **Wi-Fi Deauthentication, RF Signal Cloning/Replay, and Signal Jamming** capabilities) are provided strictly for **educational purposes, ethical hacking, and authorized security auditing only**.

By downloading, installing, or using this project, you acknowledge and agree to the following terms:

1. **Authorized Use Only:** You may only use these tools on networks, devices, and systems that you legally own or have explicit, written permission to test.
2. **Strict Compliance with Local Laws:** The use of certain features—especially Signal Jamming and RF interception—is strictly regulated and considered illegal in many jurisdictions worldwide (e.g., FCC regulations in the US, MIC regulations in Vietnam). It is your absolute responsibility to understand and comply with all local, state, and federal laws before utilizing this software or hardware.
3. **No Malicious Intent:** You agree not to use this project for any illegal, malicious, or unethical activities, including causing intentional denial of service (DoS), unauthorized access, property damage, or intercepting private communications.
4. **No Warranty and Zero Liability:** This project is provided "AS IS", without any warranties of any kind, express or implied. The author(s), contributors, and maintainers of this repository assume **NO LIABILITY** and are **NOT RESPONSIBLE** for any misuse, physical or digital damage, data loss, or legal consequences caused directly or indirectly by the use of this software.

**USE AT YOUR OWN RISK.** If you do not agree with these terms, you must immediately delete this software and cease using the hardware.
